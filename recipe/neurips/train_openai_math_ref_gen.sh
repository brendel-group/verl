...
# set both to -1.0 to not clip the ratio
clip_ratio_low=-1.0
clip_ratio_high=-1.0
...
USE_KL_LOSS=False
USE_REF_FOR_GENERATION=True

if [ ${clip_ratio_low} == -1.0 ] && [ ${clip_ratio_high} == -1.0 ]; then
    exp_name="${MODEL_PATH}_${dataset_name}_n_${n_resp_per_prompt}_bsz_${train_prompt_bsz}_epochs_${num_epochs}_kl_coef_${kl_coef}_use_ref_for_generation_${USE_REF_FOR_GENERATION}_no_clipping"
else
    exp_name="${MODEL_PATH}_${dataset_name}_n_${n_resp_per_prompt}_bsz_${train_prompt_bsz}_epochs_${num_epochs}_kl_coef_${kl_coef}_use_ref_for_generation_${USE_REF_FOR_GENERATION}_clip_ratio_low_${clip_ratio_low}_clip_ratio_high_${clip_ratio_high}"
fi

#MODEL_PATH=/fast/pmayilvahanan/post_training/verl_checkpoints/dapo/DAPO-Qwen2.5-7B-Math-DAPO/global_step_8
resume_mode=auto
resume_from_path=False
CKPTS_DIR=${CKPTS_DIR:-"${RAY_DATA_HOME}/post_training/verl_checkpoints/self_distillation_neurips/${exp_name}"}
...
python3 -m verl.trainer.main_ppo \
    ...
    data.val_files=[/fast/pmayilvahanan/datasets/openai_math/test.parquet,/fast/pmayilvahanan/datasets/aime_2024/test.parquet] \
    ...
    trainer.use_ref_for_generation=${USE_REF_FOR_GENERATION} \
