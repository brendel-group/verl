...
if [ -n "${IDENTIFIER}" ]; then
    EXPNAME="${MODEL_PATH}_${IDENTIFIER}_entropy_${ENTROPY_COEF}_kl_${KL_LOSS_COEF}_${TIMESTAMP}"
else
...
# however, they point to features added with recent verl updates. We're working with an
# old verl version.
# ─────────────────────────────────────────────────────────────────────────────
